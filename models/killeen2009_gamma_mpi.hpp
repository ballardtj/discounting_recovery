...
namespace killeen2009_gamma_mpi_model_namespace {
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "/QRISdata/Q0992/models/killeen2009_gamma_mpi.stan");
    reader.add_event(126, 124, "end", "/QRISdata/Q0992/models/killeen2009_gamma_mpi.stan");
    return reader;
}
...
likelihood(const Eigen::Matrix<T0__, Eigen::Dynamic,1>& phi,
    ...
    try {
        ...
        stan::math::assign(lambda,get_base1(theta,3,"theta",1));
        ...
        stan::math::assign(sigma,(get_base1(phi,3,"phi",1) + (get_base1(phi,4,"phi",1) * get_base1(theta,4,"theta",1))));
        ...
        for (int i = 1; i <= Nvalid; ++i) {
            ...
            stan::math::assign(u_a, (pow(m_a,alpha) - (lambda * pow(d_a,beta))));
            current_statement_begin__ = 39;
            stan::math::assign(u_b, (pow(m_b,alpha) - (lambda * pow(d_b,beta))));
...
class killeen2009_gamma_mpi_model : public prob_grad {
private:
    int Nsubj;
...
public:
    killeen2009_gamma_mpi_model(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "killeen2009_gamma_mpi_model_namespace::killeen2009_gamma_mpi_model";
        ...
        try {
            ...
            validate_non_negative_index("lambda", "Nsubj", Nsubj);
            num_params_r__ += Nsubj;
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        try {
            writer__.scalar_lub_unconstrain(0,1,alpha_mean);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable alpha_mean: ") + e.what());
        }

        if (!(context__.contains_r("alpha_prec")))
            throw std::runtime_error("variable alpha_prec missing");
        vals_r__ = context__.vals_r("alpha_prec");
        pos__ = 0U;
        context__.validate_dims("initialization", "alpha_prec", "double", context__.to_vec());
        double alpha_prec(0);
        alpha_prec = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,alpha_prec);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable alpha_prec: ") + e.what());
        }
        ...
        try {
            writer__.scalar_lub_unconstrain(0,1,beta_mean);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable beta_mean: ") + e.what());
        }

        if (!(context__.contains_r("beta_prec")))
            throw std::runtime_error("variable beta_prec missing");
        vals_r__ = context__.vals_r("beta_prec");
        pos__ = 0U;
        context__.validate_dims("initialization", "beta_prec", "double", context__.to_vec());
        double beta_prec(0);
        beta_prec = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,beta_prec);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable beta_prec: ") + e.what());
        }
        ...
        if (!(context__.contains_r("lambda_shape")))
            throw std::runtime_error("variable lambda_shape missing");
        vals_r__ = context__.vals_r("lambda_shape");
        pos__ = 0U;
        context__.validate_dims("initialization", "lambda_shape", "double", context__.to_vec());
        double lambda_shape(0);
        lambda_shape = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,lambda_shape);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable lambda_shape: ") + e.what());
        }

        if (!(context__.contains_r("lambda_scale")))
            throw std::runtime_error("variable lambda_scale missing");
        vals_r__ = context__.vals_r("lambda_scale");
        pos__ = 0U;
        context__.validate_dims("initialization", "lambda_scale", "double", context__.to_vec());
        double lambda_scale(0);
        lambda_scale = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,lambda_scale);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable lambda_scale: ") + e.what());
        }
        ...
        pos__ = 0U;
        validate_non_negative_index("lambda", "Nsubj", Nsubj);
        context__.validate_dims("initialization", "lambda", "vector_d", context__.to_vec(Nsubj));
        vector_d lambda(static_cast<Eigen::VectorXd::Index>(Nsubj));
        for (int j1__ = 0U; j1__ < Nsubj; ++j1__)
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            if (jacobian__)
                alpha_mean = in__.scalar_lub_constrain(0,1,lp__);
            else
                alpha_mean = in__.scalar_lub_constrain(0,1);

            local_scalar_t__ alpha_prec;
            (void) alpha_prec;  // dummy to suppress unused var warning
            if (jacobian__)
                alpha_prec = in__.scalar_lb_constrain(0,lp__);
            else
                alpha_prec = in__.scalar_lb_constrain(0);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  alpha;
            (void) alpha;  // dummy to suppress unused var warning
            if (jacobian__)
                alpha = in__.vector_lub_constrain(0,1,Nsubj,lp__);
            else
                alpha = in__.vector_lub_constrain(0,1,Nsubj);

            local_scalar_t__ beta_mean;
            (void) beta_mean;  // dummy to suppress unused var warning
            if (jacobian__)
                beta_mean = in__.scalar_lub_constrain(0,1,lp__);
            else
                beta_mean = in__.scalar_lub_constrain(0,1);

            local_scalar_t__ beta_prec;
            (void) beta_prec;  // dummy to suppress unused var warning
            if (jacobian__)
                beta_prec = in__.scalar_lb_constrain(0,lp__);
            else
                beta_prec = in__.scalar_lb_constrain(0);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  beta;
            (void) beta;  // dummy to suppress unused var warning
            if (jacobian__)
                beta = in__.vector_lub_constrain(0,1,Nsubj,lp__);
            else
                beta = in__.vector_lub_constrain(0,1,Nsubj);

            local_scalar_t__ lambda_shape;
            (void) lambda_shape;  // dummy to suppress unused var warning
            if (jacobian__)
                lambda_shape = in__.scalar_lb_constrain(0,lp__);
            else
                lambda_shape = in__.scalar_lb_constrain(0);

            local_scalar_t__ lambda_scale;
            (void) lambda_scale;  // dummy to suppress unused var warning
            if (jacobian__)
                lambda_scale = in__.scalar_lb_constrain(0,lp__);
            else
                lambda_scale = in__.scalar_lb_constrain(0);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  lambda;
            (void) lambda;  // dummy to suppress unused var warning
            if (jacobian__)
                lambda = in__.vector_lb_constrain(0,Nsubj,lp__);
            else
                lambda = in__.vector_lb_constrain(0,Nsubj);
            ...
            for (int subj = 1; subj <= Nsubj; ++subj) {
                ...
                stan::model::assign(theta, 
                            ...
                            get_base1(lambda,subj,"lambda",1), 
                            "assigning variable theta");
            ...
            lp_accum__.add(gamma_log<propto__>(alpha_prec, 1, 20));
            current_statement_begin__ = 109;
            lp_accum__.add(gamma_log<propto__>(beta_prec, 1, 20));
            current_statement_begin__ = 111;
            lp_accum__.add(normal_log<propto__>(lambda_shape, 0, 1));
            current_statement_begin__ = 112;
            lp_accum__.add(normal_log<propto__>(lambda_scale, 0, 1));
            current_statement_begin__ = 114;
            lp_accum__.add(normal_log<propto__>(sigma_mean, 0, 1));
            current_statement_begin__ = 115;
            lp_accum__.add(normal_log<propto__>(sigma_sd, 0, 1));
            current_statement_begin__ = 117;
            lp_accum__.add(beta_log<propto__>(alpha, (alpha_mean * alpha_prec), ((1 - alpha_mean) * alpha_prec)));
            current_statement_begin__ = 118;
            lp_accum__.add(beta_log<propto__>(beta, (beta_mean * beta_prec), ((1 - beta_mean) * beta_prec)));
            current_statement_begin__ = 119;
            lp_accum__.add(gamma_log<propto__>(lambda, lambda_shape, inv(lambda_scale)));
    ...
    void get_param_names(std::vector<std::string>& names__) const {
        ...
        names__.push_back("alpha_prec");
        names__.push_back("alpha");
        names__.push_back("beta_mean");
        names__.push_back("beta_prec");
        names__.push_back("beta");
        names__.push_back("lambda_shape");
        names__.push_back("lambda_scale");
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__,params_i__);
        static const char* function__ = "killeen2009_gamma_mpi_model_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        double alpha_mean = in__.scalar_lub_constrain(0,1);
        double alpha_prec = in__.scalar_lb_constrain(0);
        vector_d alpha = in__.vector_lub_constrain(0,1,Nsubj);
        double beta_mean = in__.scalar_lub_constrain(0,1);
        double beta_prec = in__.scalar_lb_constrain(0);
        vector_d beta = in__.vector_lub_constrain(0,1,Nsubj);
        double lambda_shape = in__.scalar_lb_constrain(0);
        double lambda_scale = in__.scalar_lb_constrain(0);
        vector_d lambda = in__.vector_lb_constrain(0,Nsubj);
        ...
        vars__.push_back(alpha_prec);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
            vars__.push_back(alpha[k_0__]);
            }
        vars__.push_back(beta_mean);
        vars__.push_back(beta_prec);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
            vars__.push_back(beta[k_0__]);
            }
        vars__.push_back(lambda_shape);
        vars__.push_back(lambda_scale);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
    ...
    static std::string model_name() {
        return "killeen2009_gamma_mpi_model";
    }
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        param_name_stream__.str(std::string());
        param_name_stream__ << "alpha_prec";
        param_names__.push_back(param_name_stream__.str());
        ...
        param_name_stream__.str(std::string());
        param_name_stream__ << "lambda_shape";
        param_names__.push_back(param_name_stream__.str());
        param_name_stream__.str(std::string());
        param_name_stream__ << "lambda_scale";
        param_names__.push_back(param_name_stream__.str());
        for (int k_0__ = 1; k_0__ <= Nsubj; ++k_0__) {
            param_name_stream__.str(std::string());
...
typedef killeen2009_gamma_mpi_model_namespace::killeen2009_gamma_mpi_model stan_model;

STAN_REGISTER_MAP_RECT(1, killeen2009_gamma_mpi_model_namespace::likelihood_functor__)
