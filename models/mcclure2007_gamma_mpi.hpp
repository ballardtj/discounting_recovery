...
namespace mcclure2007_gamma_mpi_model_namespace {
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "/QRISdata/Q0992/models/mcclure2007_gamma_mpi.stan");
    reader.add_event(130, 128, "end", "/QRISdata/Q0992/models/mcclure2007_gamma_mpi.stan");
    return reader;
}
...
likelihood(const Eigen::Matrix<T0__, Eigen::Dynamic,1>& phi,
    ...
    try {
        ...
        stan::math::assign(delta,get_base1(theta,3,"theta",1));
...
class mcclure2007_gamma_mpi_model : public prob_grad {
private:
    int Nsubj;
...
public:
    mcclure2007_gamma_mpi_model(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "mcclure2007_gamma_mpi_model_namespace::mcclure2007_gamma_mpi_model";
        ...
        try {
            ...
            validate_non_negative_index("delta", "Nsubj", Nsubj);
            num_params_r__ += Nsubj;
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("omega_mean")))
            throw std::runtime_error("variable omega_mean missing");
        vals_r__ = context__.vals_r("omega_mean");
        pos__ = 0U;
        context__.validate_dims("initialization", "omega_mean", "double", context__.to_vec());
        double omega_mean(0);
        omega_mean = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(0,1,omega_mean);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable omega_mean: ") + e.what());
        }

        if (!(context__.contains_r("omega_prec")))
            throw std::runtime_error("variable omega_prec missing");
        vals_r__ = context__.vals_r("omega_prec");
        pos__ = 0U;
        context__.validate_dims("initialization", "omega_prec", "double", context__.to_vec());
        double omega_prec(0);
        omega_prec = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,omega_prec);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable omega_prec: ") + e.what());
        }
        ...
        if (!(context__.contains_r("beta_shape")))
            throw std::runtime_error("variable beta_shape missing");
        vals_r__ = context__.vals_r("beta_shape");
        pos__ = 0U;
        context__.validate_dims("initialization", "beta_shape", "double", context__.to_vec());
        double beta_shape(0);
        beta_shape = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,beta_shape);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable beta_shape: ") + e.what());
        }
        ...
        pos__ = 0U;
        context__.validate_dims("initialization", "beta_scale", "double", context__.to_vec());
        double beta_scale(0);
        beta_scale = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,beta_scale);
        } catch (const std::exception& e) { 
        ...
        if (!(context__.contains_r("delta_scale")))
            throw std::runtime_error("variable delta_scale missing");
        vals_r__ = context__.vals_r("delta_scale");
        pos__ = 0U;
        context__.validate_dims("initialization", "delta_scale", "double", context__.to_vec());
        double delta_scale(0);
        delta_scale = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,delta_scale);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable delta_scale: ") + e.what());
        }
        ...
        pos__ = 0U;
        validate_non_negative_index("delta", "Nsubj", Nsubj);
        context__.validate_dims("initialization", "delta", "vector_d", context__.to_vec(Nsubj));
        vector_d delta(static_cast<Eigen::VectorXd::Index>(Nsubj));
        for (int j1__ = 0U; j1__ < Nsubj; ++j1__)
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            local_scalar_t__ omega_mean;
            (void) omega_mean;  // dummy to suppress unused var warning
            if (jacobian__)
                omega_mean = in__.scalar_lub_constrain(0,1,lp__);
            else
                omega_mean = in__.scalar_lub_constrain(0,1);

            local_scalar_t__ omega_prec;
            (void) omega_prec;  // dummy to suppress unused var warning
            if (jacobian__)
                omega_prec = in__.scalar_lb_constrain(0,lp__);
            else
                omega_prec = in__.scalar_lb_constrain(0);
            ...
            local_scalar_t__ beta_shape;
            (void) beta_shape;  // dummy to suppress unused var warning
            if (jacobian__)
                beta_shape = in__.scalar_lb_constrain(0,lp__);
            else
                beta_shape = in__.scalar_lb_constrain(0);

            local_scalar_t__ beta_scale;
            (void) beta_scale;  // dummy to suppress unused var warning
            if (jacobian__)
                beta_scale = in__.scalar_lb_constrain(0,lp__);
            else
                beta_scale = in__.scalar_lb_constrain(0);
            ...
            local_scalar_t__ delta_scale;
            (void) delta_scale;  // dummy to suppress unused var warning
            if (jacobian__)
                delta_scale = in__.scalar_lb_constrain(0,lp__);
            else
                delta_scale = in__.scalar_lb_constrain(0);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  delta;
            (void) delta;  // dummy to suppress unused var warning
            if (jacobian__)
                delta = in__.vector_lb_constrain(0,Nsubj,lp__);
            else
                delta = in__.vector_lb_constrain(0,Nsubj);
            ...
            lp_accum__.add(gamma_log<propto__>(omega_prec, 1, 20));
            current_statement_begin__ = 111;
            lp_accum__.add(normal_log<propto__>(beta_shape, 0, 1));
            current_statement_begin__ = 112;
            lp_accum__.add(normal_log<propto__>(beta_scale, 0, 1));
            current_statement_begin__ = 114;
            lp_accum__.add(normal_log<propto__>(delta_shape, 0, 1));
            current_statement_begin__ = 115;
            lp_accum__.add(normal_log<propto__>(delta_scale, 0, 1));
            current_statement_begin__ = 117;
            lp_accum__.add(normal_log<propto__>(sigma_mean, 0, 1));
            current_statement_begin__ = 118;
            lp_accum__.add(normal_log<propto__>(sigma_sd, 0, 1));
            current_statement_begin__ = 120;
            lp_accum__.add(beta_log<propto__>(omega, (omega_mean * omega_prec), ((1 - omega_mean) * omega_prec)));
            current_statement_begin__ = 121;
            lp_accum__.add(gamma_log<propto__>(beta, beta_shape, inv(beta_scale)));
            current_statement_begin__ = 122;
            lp_accum__.add(gamma_log<propto__>(delta, delta_shape, inv(delta_scale)));
    ...
    void get_param_names(std::vector<std::string>& names__) const {
        names__.resize(0);
        names__.push_back("omega_mean");
        names__.push_back("omega_prec");
        names__.push_back("omega");
        names__.push_back("beta_shape");
        names__.push_back("beta_scale");
        names__.push_back("beta");
        names__.push_back("delta_shape");
        names__.push_back("delta_scale");
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__,params_i__);
        static const char* function__ = "mcclure2007_gamma_mpi_model_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        double omega_mean = in__.scalar_lub_constrain(0,1);
        double omega_prec = in__.scalar_lb_constrain(0);
        vector_d omega = in__.vector_lub_constrain(0,1,Nsubj);
        double beta_shape = in__.scalar_lb_constrain(0);
        double beta_scale = in__.scalar_lb_constrain(0);
        vector_d beta = in__.vector_lb_constrain(0,Nsubj);
        double delta_shape = in__.scalar_lb_constrain(0);
        double delta_scale = in__.scalar_lb_constrain(0);
        vector_d delta = in__.vector_lb_constrain(0,Nsubj);
        ...
        vars__.push_back(omega_mean);
        vars__.push_back(omega_prec);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
            vars__.push_back(omega[k_0__]);
            }
        vars__.push_back(beta_shape);
        vars__.push_back(beta_scale);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
            vars__.push_back(beta[k_0__]);
            }
        vars__.push_back(delta_shape);
        vars__.push_back(delta_scale);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
    ...
    static std::string model_name() {
        return "mcclure2007_gamma_mpi_model";
    }
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        param_name_stream__ << "omega_mean";
        param_names__.push_back(param_name_stream__.str());
        param_name_stream__.str(std::string());
        param_name_stream__ << "omega_prec";
        param_names__.push_back(param_name_stream__.str());
        ...
        param_name_stream__.str(std::string());
        param_name_stream__ << "beta_shape";
        param_names__.push_back(param_name_stream__.str());
        param_name_stream__.str(std::string());
        param_name_stream__ << "beta_scale";
        param_names__.push_back(param_name_stream__.str());
        for (int k_0__ = 1; k_0__ <= Nsubj; ++k_0__) {
            param_name_stream__.str(std::string());
        ...
        param_names__.push_back(param_name_stream__.str());
        param_name_stream__.str(std::string());
        param_name_stream__ << "delta_scale";
        param_names__.push_back(param_name_stream__.str());
        for (int k_0__ = 1; k_0__ <= Nsubj; ++k_0__) {
            param_name_stream__.str(std::string());
...
typedef mcclure2007_gamma_mpi_model_namespace::mcclure2007_gamma_mpi_model stan_model;

STAN_REGISTER_MAP_RECT(1, mcclure2007_gamma_mpi_model_namespace::likelihood_functor__)
