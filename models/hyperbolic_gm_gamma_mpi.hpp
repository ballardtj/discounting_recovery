...
namespace hyperbolic_gm_gamma_mpi_model_namespace {
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "/QRISdata/Q0992/models/hyperbolic_gm_gamma_mpi.stan");
    reader.add_event(115, 113, "end", "/QRISdata/Q0992/models/hyperbolic_gm_gamma_mpi.stan");
    return reader;
}
...
likelihood(const Eigen::Matrix<T0__, Eigen::Dynamic,1>& phi,
    ...
    try {
        ...
        stan::math::assign(s,get_base1(theta,2,"theta",1));
...
class hyperbolic_gm_gamma_mpi_model : public prob_grad {
private:
    int Nsubj;
...
public:
    hyperbolic_gm_gamma_mpi_model(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "hyperbolic_gm_gamma_mpi_model_namespace::hyperbolic_gm_gamma_mpi_model";
        ...
        try {
            ...
            validate_non_negative_index("s", "Nsubj", Nsubj);
            num_params_r__ += Nsubj;
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("s_shape")))
            throw std::runtime_error("variable s_shape missing");
        vals_r__ = context__.vals_r("s_shape");
        pos__ = 0U;
        context__.validate_dims("initialization", "s_shape", "double", context__.to_vec());
        double s_shape(0);
        s_shape = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,s_shape);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable s_shape: ") + e.what());
        }

        if (!(context__.contains_r("s_scale")))
            throw std::runtime_error("variable s_scale missing");
        vals_r__ = context__.vals_r("s_scale");
        pos__ = 0U;
        context__.validate_dims("initialization", "s_scale", "double", context__.to_vec());
        double s_scale(0);
        s_scale = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,s_scale);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable s_scale: ") + e.what());
        }
        ...
        pos__ = 0U;
        validate_non_negative_index("s", "Nsubj", Nsubj);
        context__.validate_dims("initialization", "s", "vector_d", context__.to_vec(Nsubj));
        vector_d s(static_cast<Eigen::VectorXd::Index>(Nsubj));
        for (int j1__ = 0U; j1__ < Nsubj; ++j1__)
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            local_scalar_t__ s_shape;
            (void) s_shape;  // dummy to suppress unused var warning
            if (jacobian__)
                s_shape = in__.scalar_lb_constrain(0,lp__);
            else
                s_shape = in__.scalar_lb_constrain(0);

            local_scalar_t__ s_scale;
            (void) s_scale;  // dummy to suppress unused var warning
            if (jacobian__)
                s_scale = in__.scalar_lb_constrain(0,lp__);
            else
                s_scale = in__.scalar_lb_constrain(0);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  s;
            (void) s;  // dummy to suppress unused var warning
            if (jacobian__)
                s = in__.vector_lb_constrain(0,Nsubj,lp__);
            else
                s = in__.vector_lb_constrain(0,Nsubj);
            ...
            lp_accum__.add(normal_log<propto__>(s_shape, 0, 1));
            current_statement_begin__ = 102;
            lp_accum__.add(normal_log<propto__>(s_scale, 0, 1));
            ...
            lp_accum__.add(gamma_log<propto__>(s, s_shape, inv(s_scale)));
    ...
    void get_param_names(std::vector<std::string>& names__) const {
        ...
        names__.push_back("k");
        names__.push_back("s_shape");
        names__.push_back("s_scale");
        names__.push_back("s");
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__,params_i__);
        static const char* function__ = "hyperbolic_gm_gamma_mpi_model_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        ...
        double s_shape = in__.scalar_lb_constrain(0);
        double s_scale = in__.scalar_lb_constrain(0);
        vector_d s = in__.vector_lb_constrain(0,Nsubj);
        ...
        vars__.push_back(s_shape);
        vars__.push_back(s_scale);
            for (int k_0__ = 0; k_0__ < Nsubj; ++k_0__) {
    ...
    static std::string model_name() {
        return "hyperbolic_gm_gamma_mpi_model";
    }
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_shape";
        param_names__.push_back(param_name_stream__.str());
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_scale";
        param_names__.push_back(param_name_stream__.str());
        for (int k_0__ = 1; k_0__ <= Nsubj; ++k_0__) {
            param_name_stream__.str(std::string());
...
typedef hyperbolic_gm_gamma_mpi_model_namespace::hyperbolic_gm_gamma_mpi_model stan_model;

STAN_REGISTER_MAP_RECT(1, hyperbolic_gm_gamma_mpi_model_namespace::likelihood_functor__)
