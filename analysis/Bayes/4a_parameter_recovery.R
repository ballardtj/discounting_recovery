...
#load fit object
load(file=paste0("data/derived/fits/manos_fit_Bayes_", names(models)[i],".RData"))
...
#for(j in 1:nsamples){
  ...
  save(fit,file=paste0("data/derived/recovery/manos_recovery_fit_",names(models)[i],"_",j,".RData"))

#}

#save generating parameters
#save(parms,file=paste0("data/derived/recovery_generating_parms_",names(models)[i],".RData"))
save(parms,file=paste0("data/derived/recovery/manos_recovery_generating_parms_",names(models)[i],"_",j,".RData"))
